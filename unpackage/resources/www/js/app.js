...
// 		}
		...
		mui.ajax('http://192.168.0.110:8080/Login/LoginServlet', {
			type: 'POST',
			data: loginInfo,
			...
			success: function(user) {
				if (user) {
					var users = JSON.parse(localStorage.getItem('$users') || '[]');
					users=user;
					localStorage.setItem('$users', JSON.stringify(user));/*用户信息的本地存储*/
					return owner.createState(loginInfo.username, callback);
				} else {
					return callback('用户名或密码错误!');
				}
	...
	owner.alter = function(alterInfo, callback) {
		mui.ajax('http://192.168.0.110:8080/Login/PwdUpdateServlet', {
			type: 'POST',
			data: alterInfo,
			dataType: 'json', //服务器返回json格式数据
			timeout: 15000, //15秒超时
